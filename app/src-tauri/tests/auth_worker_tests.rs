...
use shorts_tauri_app::auth_worker::{
    activation_idempotency_key, build_worker_client, reset_idempotency_key, LicenseStore,
    PolicyWorkerClient, WorkerActivationContract, WorkerCircuitBreakerPolicy, WorkerClientPolicy,
    WorkerContractErrorCode, WorkerResetContract, WorkerRetryPolicy,
};
use shorts_tauri_app::core::config::{LicenseBackendMode, LicenseWorkerConfig};
...
async fn mock_backend_mode_routes_through_local_worker_switch() {
    let cfg = LicenseWorkerConfig {
        backend_mode: LicenseBackendMode::Mock,
        base_url: String::new(),
...
async fn hosted_worker_mode_hits_real_local_http_route_contract() {
    ...
    let cfg = LicenseWorkerConfig {
        backend_mode: LicenseBackendMode::Hosted,
        base_url,
        storage_namespace: "desktop-client-test".to_string(),
        keychain_service: "shorts-test".to_string(),
        timeout_ms: 1000,
